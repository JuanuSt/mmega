...
get_accounts_stat() {
 ...
 if [ $accounts_err != 0 ] && [ "${SEARCH_MODE}" = "on" ]; then
    ...
    status="$circle_blk"
    printf " %-16s %-15s" " $status" "$accounts_con/$accounts_total_checked connected"
    printf " %5s %-3s %5s %-3s %5s %-3s %5s %-3s %5s %-3s\n" "${txtgrn}sync" "$accounts_sync" "${txtylw}unsyn" "$accounts_not_sync" "${txtred}fail" "$accounts_fail" "${txtblu}empty" "$accounts_empty" "${txtblk}error" "$accounts_err${txtrst}"
    ...
    status="$circle_grn"
    printf " %-16s %-15s" " $status" "$accounts_con/$accounts_total_checked connected"
    printf " %5s %-3s %5s %-3s %5s %-3s %5s %-3s\n" "${txtgrn}sync" "$accounts_sync" "${txtylw}unsyn" "$accounts_not_sync" "${txtred}fail" "$accounts_fail" "${txtblu}empty" "$accounts_empty${txtrst}"
