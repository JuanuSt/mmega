...
VERSION=0.9
SUBJECT=mmega_cron
USAGE="mmega_cron -c <file> -m <mode> [options]\n       mmega_cron -s <dir> <name> [options]"
...
# -- Symbols with colors ------------------------------------------
up_arrow_red="${up_arrow}"
down_arrow_grn="${down_arrow}"

down_arrow_blu="${down_arrow}"
up_arrow_blu="${up_arrow}"

varrows_down_blk="${varrows_down}"
point_blk="${point}"

varrows_down_bold="${varrows_down}"
hrarrows_blu_bold="${hrarrows}"
...
clear
echo
echo -e "   MEGAtools multi-account ${kass}                                      v.$VERSION"
echo -e "                                                       $hour $watch $date"
...
set_tmp_file(){
 ...
 if [ "$conf_extension" = "gpg" ] && [ "$conf_file_type" = "RSA" ]; then  ## only for RSA encrypted files
    echo -ne "${plus} Config file [$conf_filename] "
    ...
    if [[ -z "$tmp_conf_file" ]]; then
       ...
       echo -e "[·] The file was encrypted by $MAIL_KEY [Key ID $GPG_KEY]"
       echo
       exit 1
    fi
 ...
 ## tmp file mode
    ...
    echo -e "${plus} Config file populated with accounts [$conf_filename]"
 ...
 ## unencrypted mode
    ...
    echo -e "${excl} Config file [$conf_filename]"
    tmp_conf_file=$(cat $CONF_FILE)
 ...
 ## file not valid
    ...
    echo -e "${minus} File not valid [$conf_filename]"
    echo
    exit 1
 fi
}
...
check_empty_lines() {
 ...
 if [[ $empty_lines != 0 ]]; then
    echo -e "${excl} $empty_lines empty lines deleted in config file"
 ...
 for line in $tmp_conf_file;do
    ...
    # Checking account mail
      if [[ ! "$username" = "$( echo $username | grep '[a-z, 0-9]@[a-z, 0-9]*...[a-z, 0-9]' )" ]];then
         ...
         accounts_err=$(( $accounts_err + 1 ))
         echo -e "${excl} Wrong username [in $name]"
    # Check empty fields (username, passwd)
      elif [[ -z "$username" ]] || [[ -z "$passwd" ]];then
         tmp_conf_file=$( echo "$tmp_conf_file" | grep -v "$line")
         accounts_err=$(( $accounts_err + 1 ))
         echo -e "${excl} Empty username/password [in $name]"
      fi
 done
}
...
get_accounts_stat() {
 ...
 if [ $accounts_err != 0 ] && [ "${SEARCH_MODE}" = "on" ]; then
    status="$circle_blk"
    printf " %-5s %-15s" " $status" "$accounts_con/$accounts_total_checked connected"
    printf " %4s %-3s\n"  "error" "$accounts_err "
 elif [ "${SEARCH_MODE}" = "on" ]; then
    status="$circle_grn"
    ...
    status="$circle_blk"
    printf " %-5s %-15s" " $status" "$accounts_con/$accounts_total_checked connected"
    printf " %5s %-3s %5s %-3s %5s %-3s %5s %-3s %5s %-3s\n" "sync" "$accounts_sync" "unsyn" "$accounts_not_sync" "fail" "$accounts_fail" "empty" "$accounts_empty" "error" "$accounts_err"
 elif [ $accounts_fail != 0 ]; then
    status="$circle_ylw"
    printf " %-5s %-15s" " $status" "$accounts_con/$accounts_total_checked connected"
    printf " %5s %-3s %5s %-3s %5s %-3s %5s %-3s\n" "sync" "$accounts_sync" "unsyn" "$accounts_not_sync" "fail" "$accounts_fail"
 elif [ $accounts_empty != 0 ]; then
    status="$circle_grn"
    printf " %-5s %-15s" " $status" "$accounts_con/$accounts_total_checked connected"
    printf " %5s %-3s %5s %-3s %5s %-3s %5s %-3s\n" "sync" "$accounts_sync" "unsyn" "$accounts_not_sync" "fail" "$accounts_fail" "empty" "$accounts_empty"
 elif [ $accounts_not_sync != 0 ]; then
    status="$circle_grn"
    printf " %-5s %-15s" " $status" "$accounts_con/$accounts_total_checked connected"
    printf " %5s %-3s %5s %-3s\n" "sync" "$accounts_sync" "unsyn" "$accounts_not_sync "
 else
    status="$circle_grn"
    printf " %-5s %-15s" " $status" "$accounts_con/$accounts_total_checked connected"
    printf " %5s %-3s\n" "sync" "$accounts_sync "
...
# encrypted
 ...
 #file not valid
   ...
   #Upload
   if [[ "$num_to_up" != 0  ]]; then
      printf " %-4s %-40s\n" "$up_arrow_red" " $num_to_up files to be uploaded"
      echo -e "$files_to_up"
   fi

   #Download
   if [[ "$num_to_down" != 0  ]];then
      printf " %-4s %-40s\n" "$down_arrow_grn" " $num_to_down files to be downloaded"
...
show_files(){
 if [[ "$accounts_not_sync" == 0 ]] && [[ $accounts_fail != 0  ||  $accounts_err != 0 ]];then
    printf "  %-4s %-40s\n" "$right_arrow" " No files to sycn but $(( $accounts_fail + $accounts_err )) accounts failed to be check"
    echo
 elif [[ $accounts_not_sync != 0 ]] && [[ $accounts_fail != 0 ||  $accounts_err != 0 ]];then
    printf "  %-4s %-40s\n" "$right_arrow" " $(( $accounts_fail + $accounts_err )) accounts failed. No files will be shown"
    echo
...
#Hostname
echo -e "${plus} Host $HOST"


#if search mode on aqui
# -- MODE SEARCH ----------------------------------------------------------------------------------------------------------
if [[ "${SEARCH_MODE}" = "on" ]]; then
   echo -e "${plus} Searching $SEARCH_NAME config files in $SEARCH_DIR"
   ...
   echo -ne "${plus} Checking mega files "
   declare_variables

   if [ $accounts_num != $acc_t1 ];then
      echo -e "[$accounts_num accounts $accounts_err errors ]"
   else
      echo -e "[$accounts_num accounts]"
   fi
   echo
...
if [[ "${CHECK_MODE}" = "df" ]];then
   echo -ne "${plus} Checking disk free "
   declare_variables
...
## Check mode DOWNLOAD ----------------------------------------------
   ...
   #download
   if [[ $accounts_not_sync = 0 ]] && [[ $accounts_fail != 0 || $accounts_err != 0 ]];then
      printf "  %-4s %-40s\n" "$right_arrow" " All accounts sycn but $(( $accounts_fail + $accounts_err )) accounts failed to be check."
   elif [[ $accounts_not_sync != 0 ]] && [[ $accounts_fail != 0 || $accounts_err != 0 ]];then
      printf "  %-4s %-40s\n" "$right_arrow" " $(( $accounts_fail + $accounts_err )) accounts failed to be check and not will downloaded. Downloading accounts..."
      echo
      for line in $acc_to_sync;do
         IFS=, read name username passwd local_dir remote_dir trash <<< "$line"
         cut_name
         ...
         if [[ "$num_to_down" != 0  ]];then
            printf " %-4s %-60s\n" "${down_arrow_grn}" " Downloading $name                                           "
            download
            printf " %-4s %-60s\n" "${down_arrow_blu}" " Done                                                               "
            echo
         fi
      ...
      printf "  %-4s %-40s\n" "$right_arrow" " Downloading accounts..."
      echo
      for line in $acc_to_sync;do
         IFS=, read name username passwd local_dir remote_dir trash <<< "$line"
         cut_name
      ...
      printf "  %-4s %-40s\n" "$right_arrow" " All accounts are synchronized. Nothing to download."
   fi


## Check mode UPLOAD ----------------------------------------------
elif [[ "${CHECK_MODE}" = "up" ]]; then
   echo -ne "${plus} Uploading "
   declare_variables
   ...
   if [[ $accounts_not_sync = 0 ]] && [[ $accounts_fail != 0 || $accounts_err != 0 ]];then
      ...
      printf "  %-4s %-40s\n" "$right_arrow" " $(( $accounts_fail + $accounts_err )) accounts failed to be check and not will uploaded. Uploading accounts..."
      echo
      for line in $acc_to_sync;do
         IFS=, read name username passwd local_dir remote_dir trash <<< "$line"
         cut_name
      ...
      printf "  %-4s %-40s\n" "$right_arrow" " Uploading accounts..."
      echo
      for line in $acc_to_sync;do
         IFS=, read name username passwd local_dir remote_dir trash <<< "$line"
         cut_name
      ...
      printf "  %-4s %-40s\n" "$right_arrow" " All accounts are synchronized. Nothing to upload."
   fi

## Check mode SYNC ----------------------------------------------
elif [[ "${CHECK_MODE}" = "sync" ]]; then
   echo -ne "${plus} Synchronizing "
   declare_variables
   ...
   if [[ $accounts_not_sync = 0 ]] && [[ $accounts_fail != 0 || $accounts_err != 0 ]];then
      ...
      printf "  %-4s %-40s\n" "$right_arrow" " $(( $accounts_fail + $accounts_err )) accounts failed to be check and not will synchronized"
      printf "  %-4s %-40s\n" "$right_arrow" " Synchroning accounts..."
      echo
      for line in $acc_to_sync;do
         IFS=, read name username passwd local_dir remote_dir trash <<< "$line"
         cut_name
      ...
      printf "  %-4s %-40s\n" "$right_arrow" " All accounts are synchronized"
   fi
